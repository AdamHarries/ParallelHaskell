...
--snIteration :: (Ord a) -> V.Vector a -> V.Vector a
--snIteration v = V.imap cas where 
--    cas :: Int -> a -> a
--    cas ix val = let (iv, ov) = (v V.! ix, v V.! (pair ix))

-- given an n, give the n'th triangle number
tri :: Int -> Int
tri n = n * (n + 1) `div` 2

-- given an n, give the triangle numbers bounding n
untri :: Int -> (Int, Int)
untri n = let ftri = (-1 + sqrt (1 + (8 * (fromIntegral n)))) / 2 in 
    (floor ftri, ceiling ftri)

iterDimsTri iter = let (lo, hi) = untri iter in
    (lo, lo - (iter - (tri lo)))
